...
- 原文自动分章：识别中文章节、Markdown 标题和英文 Chapter 标题
- 章节化写作工作台：通过章节目录选择、查看和编辑任意章节
- 小说项目持久化：项目、章节、写作要求和生成版本保存在 SQLite 中
...
- 安全重写：AI 重写或人工编辑都会创建章节新版本，失败不会丢失原稿
- 指定章节重写：选择任意原文章节或续写章节，按本次要求生成新版本
- 版本感知记忆：每个版本保存生成前后记忆快照，恢复时按活动章节链重建
- 版本恢复：每章独立保留完整历史版本，可在网页中恢复
...
| `recent_context_chars` | 12000 | 保留的近期已接受章节窗口 |
...
每个模型的 `generate_cfg.request_timeout` 默认是 120 秒。模型接口在这段
时间内没有返回数据时，本次调用会超时并在页面与控制台显示错误；标准模式
下若只有“章节规划”失败，程序会跳过计划继续生成正文，不会一直卡在规划页面。

运行时控制台会按生成任务显示项目、章节、上下文字数、模型请求 ID、首段内容
延迟、生成进度、各阶段耗时、异常堆栈和项目锁释放状态。排查模型服务问题时，
可以根据同一个 `task` 和 `id` 串联一轮续写的完整日志。

## 章节与版本

新项目会把导入内容保存为“原稿章节”。后续可以：

- 自动按 `第X章/回/卷/篇`、Markdown 标题或 `Chapter N` 拆分原文
- 从章节目录选择和查看任意章节
- 创建空白章节并人工编辑
- 让 AI 生成下一章
- 按具体要求让 AI 重写当前原文章节或续写章节
- 人工修改后保存为新版本
- 恢复任意章节的历史版本

每个章节版本记录其来源、写作计划、一致性报告，以及生成前后的长期记忆快照。恢复、删除或修改章节后，项目长期记忆会根据当前活动章节链重新同步。旧版数据库会在首次启动时自动迁移，原有正文和续写版本不会被删除。

AI 在小说末尾生成的新章节会自动在标题后添加 `(续写)`，以便和导入的原文章节区分。没有可识别章节标题的原文仍会作为一个完整章节导入。
...
| `GET` | `/restart/<project_id>` | 重写最后一章 SSE |
| `POST` | `/api/projects/<project_id>/chapters` | 创建章节 |
| `PUT` | `/api/projects/<project_id>/chapters/<chapter_id>` | 编辑章节并保存新版本 |
| `DELETE` | `/api/projects/<project_id>/chapters/<chapter_id>` | 删除非原稿章节 |
| `GET` | `/api/projects/<project_id>/chapters/<chapter_id>/generate` | AI 重写指定章节 SSE |
| `GET` | `/api/projects` | 列出当前浏览器的项目 |
| `GET` | `/api/projects/<project_id>` | 获取项目和版本 |
| `POST` | `/api/projects/<project_id>/restore/<version_id>` | 恢复章节历史版本 |
...
- 一致性检查是辅助提示，不会自动改写正文
- 修改较早章节后需要重新提炼当前活动章节链，超长项目可能产生额外模型调用
